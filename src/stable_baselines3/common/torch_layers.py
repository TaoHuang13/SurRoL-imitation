...
class Transformer(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 512):
        ...
        self.st = SwinTransformer(image_size=image_size,
                                in_chans=n_input_channels,
                                )

        # Compute shape by doing one forward pass
        with th.no_grad():
            n_flatten = self.st.forward_features(th.as_tensor(observation_space.sample()[None]).float()).shape[1]

        self.linear = nn.Sequential(nn.Linear(n_flatten, features_dim), nn.ReLU())

    def forward(self, observations: th.Tensor) -> th.Tensor:
        return self.linear(self.st.forward_features(observations))
