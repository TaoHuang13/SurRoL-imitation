...
class NeedlePick(PsmEnv):
    ...
    def _env_setup(self):
        ...
        #                     globalScaling=self.SCALING)
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'needle/needle_40mm.urdf'),
                            (workspace_limits[0].mean() + (0.5 - 0.5) * 0.1,  # TODO: scaling
                             workspace_limits[1].mean() + (0.5 - 0.5) * 0.1,
                             workspace_limits[2][0] + 0.01),
                            p.getQuaternionFromEuler((0, 0, yaw)),
                            useFixedBase=False,
                            globalScaling=self.SCALING)
    ...
    def _sample_goal(self) -> np.ndarray:
        ...
        #                  workspace_limits[1].mean() + 0.01 * np.random.randn() * self.SCALING,
        #                  workspace_limits[2][1] - 0.04 * self.SCALING])
        goal = np.array([workspace_limits[0].mean() + 0.01 * 1 * self.SCALING,
                         workspace_limits[1].mean() + 0.01 * 1 * self.SCALING,
                         workspace_limits[2][1] - 0.04 * self.SCALING])
        return goal.copy()
